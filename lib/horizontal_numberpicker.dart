...
class HorizontalNumberPicker extends StatefulWidget {
  ...
  HorizontalNumberPicker({
    ...
    this.indicatorColor = const Color.fromRGBO(92, 205, 216, 1),
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Stack(
        ...
        children: <Widget>[
          Container(
            margin: EdgeInsets.only(top: longLine),
...
class MyPainter extends CustomPainter {
  ...
  void drawLine(Canvas canvas, Size size) {
    ...
    //绘制横线
    canvas.drawLine(
        Offset(startX, longLine), Offset(endX, longLine), _linePaint);

    //绘制竖线
    for (double x = startX; x <= endX; x += subGridWidth) {
      if (x == size.width / 2) {
        //中间为长刻度
        canvas.drawLine(Offset(x, 0), Offset(x, longLine), _linePaint);
      } else {
        //其他为短刻度
        canvas.drawLine(Offset(x, 0), Offset(x, shortLine), _linePaint);
      }
    }
  }
  ...
  void drawText(Canvas canvas, Size size) {
    ...
    canvas.drawParagraph(p, Offset(size.width / 2 - halfWidth, longLine + 4));
  }
